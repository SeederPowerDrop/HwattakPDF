...
> **0.9.0 (build 19)**의 최신 소스 검증은 [9월 23일 소스 갱신 기록](SOURCE-UPDATE-2026-09-23.md), 이름 통일 전 로컬 패키지는 [9월 22일 배포 검증 기록](RELEASE-VALIDATION-0.9.0.md)을 따릅니다. 아래 수치와 해시는 기존 공개 0.8.0의 역사적 기준선입니다.
...
이 문서는 공개된 0.8.0 (build 18), commit `1584312`의 자동 검증과 공개 당시 남은 한계를 기록한 고정 기준선이다. 이후 추가되어 0.9.0에 포함된 이미지·HTML·Office 변환 작업은 아래 테스트 수·산출물·해시에 포함되지 않는다. 2026-09-22에 준비한 0.9.0 (build 19) Developer Preview 로컬 패키지는 별도의 [배포 검증 기록](RELEASE-VALIDATION-0.9.0.md)을 따른다. 과거 0.7 마일스톤의 테스트 수·성능 측정·산출물 해시는 재사용하지 않았다. 수동 제품 검증은 [RELEASE-CHECKLIST.md](RELEASE-CHECKLIST.md)를 함께 사용한다.
