...
- [ ] 열기 패널, Finder drop, 명령행 인자 및 Finder 더블클릭으로 PDF 열기
...
- [ ] 라이트·다크·시스템 설정을 선택하고 문서·보조 창 전환 및 재실행 후 설정 유지 확인
- [ ] PDF 전용 아이콘의 시스템 등록과 Finder 표시를 구분해 확인; 캐시·Quick Look 미리보기 차이 기록
...
> 0.9.0의 웹 패널은 번역·웹 브라우저 2종이며, 문서 명령 플러그인 3종을 포함해 기본 검토본은 총 5종이다. 아래 YouTube 항목은 비활성 실험 기능의 향후 배포 조건이며 현재 ZIP의 출고 조건이 아니다.
...
- [ ] JPEG·PNG·TIFF·HEIC·HEIF·GIF·BMP·WebP를 각각 탭에서 열고 PDF 만들기에 추가. GIF·다중 페이지 TIFF는 실제 포함되는 프레임/페이지를 기록
- [ ] 이미지 파일당 64 MiB·원본 1억 2천만 픽셀 상한의 경계값과 초과값을 검사하고, HTML 연결 자원에는 같은 사전 합산 제한이 없다는 한계를 장시간 fixture로 확인
- [ ] 여러 이미지, 다중 페이지 HTML, 선택한 PDF 페이지만, A4 공백을 섞어 drag·버튼 재정렬·삭제한 순서대로 저장
- [ ] 암호·추출 제한 PDF 페이지 가져오기가 보안 정책에 맞게 차단되고 source와 목적지 security scope가 작업 종료·취소 뒤 해제됨
- [ ] 안정성/속도 방식의 이미지 크기, HTML 준비 대기, OCR 240/160 DPI와 정확/빠른 인식 차이가 UI 설명과 일치
- [ ] 입력 없는 상태, OCR 언어 0개, 손상 이미지/HTML/PDF, 64 MiB HTML, 500페이지 초과, timeout과 WebKit process 종료의 오류·취소 처리
- [ ] 로컬 CSS·이미지·글꼴·script가 있는 HTML과 HTTP·HTTPS 자원을 참조하는 HTML을 사용해 로컬 결과와 원격 요청 차단을 실제 네트워크 관찰로 확인
- [ ] HTML 경계의 CJK·Arabic·긴 링크·표·이미지·`break-before/after/inside`가 중복·유실 없이 A4에 배치되는지 모든 페이지를 렌더링해 확인
- [ ] 예상 시간·용량·페이지 수·신뢰도가 보장값처럼 보이지 않고 완료 뒤 실제 시간·용량·방식·OCR 여부가 올바르게 표시됨
- [ ] 저장 중 목록 변경·중복 실행 차단, 취소 뒤 부분 목적지와 임시 파일, 저장 성공 뒤 재개방 페이지 수 확인
- [ ] 생성한 DOCX/PPTX를 Microsoft Word·PowerPoint와 한 개 이상의 독립 Open XML 검사기에서 열어 페이지 수·방향·여백·혼합 페이지 크기 확인
- [ ] Office 결과가 페이지 이미지 기반이며 문단·표·도형을 편집 가능한 객체로 복원하지 않는다는 설명이 메뉴·README·개발노트에 일치
