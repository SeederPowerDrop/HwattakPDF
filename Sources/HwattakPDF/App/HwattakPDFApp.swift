...
enum MainWorkspaceWindowID: String, Codable, Hashable {
    case main
}
...
/// 객체를 한 번 만들고 하위 view에 주입한다. main 장면은 기본 workspace를,
/// UUID 기반 WindowGroup은 tear-out 탭마다 독립 workspace를 사용한다.
@main
struct HwattakPDFApp: App {
    @NSApplicationDelegateAdaptor(HwattakPDFAppDelegate.self) private var appDelegate
    ...
    var body: some Scene {
        // A WindowGroup participates in Finder's external-file routing. A
        // stable value keeps programmatic reopening attached to this workspace.
        WindowGroup("HwattakPDF", id: "main", for: MainWorkspaceWindowID.self) { _ in
            TabbedWorkspaceView(
                workspace: workspace,
                windowID: nil,
                ...
                .handlesExternalEvents(preferring: ["file:"], allowing: ["file:"])
                .onAppear {
        ...
        .commands {
            HwattakPDFCommands(preferences: preferences)
...
@MainActor
private struct HwattakPDFCommands: Commands {
    ...
    private var gridPagingDirectionValue = PDFPageNavigationMode.defaultValue.rawValue
    ...
    var body: some Commands {
        ...
        CommandGroup(after: .sidebar) {
            Menu(L10n.string("appearance.title")) {
                Picker(L10n.string("appearance.title"), selection: $preferences.appearance) {
            ...
            Menu(L10n.string("toolbar.view_layout", defaultValue: "화면 배치")) {
                ...
                Button(
                    checkedMenuTitle(
                        L10n.string("view.fit.width"),
                        isSelected: viewCommandWorkspace?.pageFitMode == .width
                    )
                ) {
                    guard let viewCommandWorkspace else { return }
                    viewCommandWorkspace.selectPageFitMode(
                        viewCommandWorkspace.pageFitMode == .width ? nil : .width
                    )
                }

                Button(
                    checkedMenuTitle(
                        L10n.string("view.fit.height"),
                        isSelected: viewCommandWorkspace?.pageFitMode == .height
                    )
                ) {
                    guard let viewCommandWorkspace else { return }
                    viewCommandWorkspace.selectPageFitMode(
                        viewCommandWorkspace.pageFitMode == .height ? nil : .height
                    )
                }
            }
            .disabled(viewCommandWorkspace?.document == nil)

            Menu(L10n.string("pdf.navigation.label")) {
                ForEach(PDFPageNavigationMode.allCases) { mode in
                    Button(
                        checkedMenuTitle(
                            mode.title,
                            isSelected: gridPagingDirectionValue == mode.rawValue
                        )
                    ) {
                        gridPagingDirectionValue = mode.rawValue
                    }
                }
            }
            .disabled(viewCommandWorkspace?.document == nil)
