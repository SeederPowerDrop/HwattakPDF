...
struct PDFGridLayoutMetrics: Equatable {
    ...
    /// Zoom relative to the ordinary overview layout, including axis fitting.
    let effectiveOverviewScale: CGFloat

    func fitsWithinViewport(_ viewportSize: CGSize) -> Bool {
        contentWidth <= max(1, viewportSize.width) + 0.5
            && contentHeight <= max(1, viewportSize.height) + 0.5
    }
    ...
    static func resolve(
        ...
        let baseTileWidth = isFourPageLayout ? min(widthFit, heightFit) : widthFit

        let safeScale = overviewScale.isFinite
            ? min(1.6, max(0.7, overviewScale))
            : 1
        let scaledTileWidth: CGFloat
        switch fitMode {
        case .width:
            scaledTileWidth = max(1, widthFit)
        case .height:
            scaledTileWidth = max(1, heightFit)
        case nil:
            scaledTileWidth = max(1, baseTileWidth * safeScale)
        }
        ...
        let contentHeight = topPadding
            ...
            + verticalSpacing * CGFloat(rows - 1)
            + bottomPadding
        let enablesHorizontalScrolling = fitMode != nil
        ...
        return PDFGridLayoutMetrics(
            ...
            effectiveOverviewScale: scaledTileWidth / max(1, baseTileWidth)
        )
    }
}
