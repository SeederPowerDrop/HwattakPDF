...
struct PDFGridOverview: View {
    ...
    @FocusState private var isPageGridFocused: Bool
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ScrollViewReader { proxy in
                ...
                .onChange(of: workspace.pageFitMode) { _, _ in
                    transientMagnification = 1
                    magnificationBaseline = nil
                    establishInitialPosition(using: proxy, metrics: metrics)
                }
                .onChange(of: geometry.size) { _, _ in
                    if workspace.pageFitMode != nil {
                        establishInitialPosition(using: proxy, metrics: metrics)
                    }
                }
                .onAppear {
            ...
            .overlay(alignment: .bottomTrailing) {
                zoomControls(metrics: metrics)
            }
            .overlay {
                PDFGridInputMonitor(
                    pagingEnabled: isFourPagePager && metrics.fitsWithinViewport(geometry.size),
                    pagingDirection: pagingDirection,
                    wheelZoomModifier: PDFWheelZoomModifier(
                        rawValue: wheelZoomModifierValue
                    ) ?? .defaultValue,
                    onMagnificationChanged: {
                        previewMagnification($0, fittedScale: metrics.effectiveOverviewScale)
                    },
                    onMagnificationEnded: {
                        commitMagnification($0, fittedScale: metrics.effectiveOverviewScale)
                    },
        ...
        .focusable()
        .focused($isPageGridFocused)
        .onKeyPress(
            keys: [.upArrow, .downArrow, .leftArrow, .rightArrow, .pageUp, .pageDown],
            phases: [.down, .repeat]
        ) { keyPress in
            guard keyPress.modifiers.intersection([.command, .control, .option, .shift]).isEmpty else {
                return .ignored
            }
            return handlePagingKey(keyPress.key)
        }
        .onDisappear {
            transientMagnification = 1
            magnificationBaseline = nil
    ...
    private func zoomControls(metrics: PDFGridLayoutMetrics) -> some View {
        HStack(spacing: 10) {
            Image(systemName: "minus.magnifyingglass")
            Slider(
                value: Binding(
                    get: { clampedOverviewScale(metrics.effectiveOverviewScale) },
                    set: { scale in
                        workspace.endPageFitForManualZoom()
                        workspace.overviewScale = scale
                    }
                ),
                in: 0.7...1.6,
                step: 0.05
            ...
            .accessibilityValue(
                L10n.format("value.percent", Int(metrics.effectiveOverviewScale * 100))
            )
    ...
    private func previewMagnification(_ factor: CGFloat, fittedScale: CGFloat) {
        guard factor.isFinite, abs(factor - 1) > 0.0001 else {
            transientMagnification = 1
            return
        }
        let baseline = magnificationBaseline ?? fittedScale
    ...
    private func commitMagnification(_ factor: CGFloat, fittedScale: CGFloat) {
        let baseline = magnificationBaseline ?? fittedScale
        transientMagnification = 1
        magnificationBaseline = nil
        guard factor.isFinite, abs(factor - 1) > 0.0001 else { return }
        let targetScale = clampedOverviewScale(baseline * factor)
        workspace.endPageFitForManualZoom()
    ...
    private func maximumPageAspect(inGroupOf groupSize: Int) -> CGFloat {
        ...
        return (start..<end).reduce(CGFloat(0.1)) { maximum, index in
    ...
    private func navigationAnchor(for metrics: PDFGridLayoutMetrics) -> UnitPoint {
        if workspace.pageFitMode == .width, metrics.contentHeight > metrics.viewportHeight {
            return .top
        }
        if workspace.pageFitMode == .height, metrics.enablesHorizontalScrolling {
            return .leading
        }
