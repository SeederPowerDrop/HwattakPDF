...
final class PDFWorkspaceState: ObservableObject {
    ...
    ///
    /// Study의 조절 가능한 투명도는 PDFKit의 표준 Highlight/Underline
    /// 직렬화로 보존되지 않는다. 따라서 각 줄을 appearance-backed Stamp로
    /// 그리되 history에는 한 묶음으로 등록하여 여러 줄도 Command-Z 한 번으로
    /// 되돌린다. Viewer의 기존 100% 하이라이트만 표준 Highlight를 유지한다.
