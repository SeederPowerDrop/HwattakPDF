...
final class StudyMarkupAnnotation: PDFAnnotation {
    ...
    override func draw(with box: PDFDisplayBox, in context: CGContext) {
        ...
        case .underline:
            // An appearance stream is clipped to the annotation bounds. Limit
            // the visible stroke to that height and move its center inward so
            // the lower half is not clipped away at the text baseline.
            let visibleWidth = min(markupThickness, max(1, bounds.height))
