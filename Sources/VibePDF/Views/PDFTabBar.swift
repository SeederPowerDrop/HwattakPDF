...
private let vibePDFTabType = UTType.utf8PlainText
...
private func tabDragSourceID(from info: DropInfo, localTabID: UUID?) -> UUID? {
    PDFTabDragPayload.resolve(
        ...
        providerSuggestedNames: info.itemProviders(for: [vibePDFTabType.identifier])
...
private func loadTabDragSourceID(
    ...
    guard let provider = info.itemProviders(for: [vibePDFTabType.identifier]).first else {
        return false
    }
...
struct PDFTabBar: View {
    ...
    var body: some View {
        ...
        .overlay {
            if isFileDropTargeted {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    ...
                    .overlay {
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            .stroke(theme.cream, style: StrokeStyle(lineWidth: 1.5, dash: [6, 4]))
                    }
                    .padding(5)
        ...
        // (brand, menus, +) so its source never remains visually "dragging".
        .onDrop(
            of: [vibePDFTabType.identifier],
    ...
    private func secondaryRow(for group: PDFTabGroup) -> some View {
        ...
        .overlay(alignment: .top) {
            Rectangle().fill(theme.cream.opacity(0.12)).frame(height: 1)
        }
    }

    private var chromeDivider: some View {
        Rectangle()
            .fill(theme.cream.opacity(0.16))
    ...
    private var brand: some View {
        HStack(spacing: 8) {
            Image(
                nsImage: iconManager.currentImage
        ...
        .layoutPriority(2)
        .accessibilityElement(children: .combine)
        .accessibilityLabel(L10n.format("app.version", VibePDFRelease.version))
    }
    ...
    private var tabSurface: some View {
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 9, style: .continuous)
                .stroke(isActive ? theme.cream.opacity(0.80) : theme.cream.opacity(0.08), lineWidth: 1)
        }
        .overlay(alignment: .bottom) {
            if isActive {
                RoundedRectangle(cornerRadius: 1)
...
private struct TabBarDragCleanupDropDelegate: DropDelegate {
    ...
    func validateDrop(info: DropInfo) -> Bool {
        draggedTabID != nil
            && info.hasItemsConforming(to: [vibePDFTabType.identifier])
    }
...
private struct TabReorderDropDelegate: DropDelegate {
    ...
    func validateDrop(info: DropInfo) -> Bool {
        ...
        return info.hasItemsConforming(to: [vibePDFTabType.identifier])
    }
