...
struct TabbedWorkspaceView: View {
    ...
    @State private var pendingExternalOpenAccesses: [SecurityScopedAccess] = []
    @State private var externalOpenCoalescingTask: Task<Void, Never>?
    ...
    private var windowTitle: String {
        guard let activeWorkspace = workspace.activeWorkspace else { return "HwattakPDF \(VibePDFRelease.badge)" }
        return !activeWorkspace.hasOpenDocument
            ? "HwattakPDF \(VibePDFRelease.badge)"
    ...
    private func enqueueExternalPDFOpen(_ url: URL) {
        ...
        externalOpenCoalescingTask = Task { @MainActor in
            ...
            // long-lived scopes before these callback scopes are released.
