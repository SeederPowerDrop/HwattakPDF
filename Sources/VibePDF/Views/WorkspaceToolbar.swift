...
struct WorkspaceToolbar: View {
    ...
    /// The four common layouts remain one-click actions. Less frequent grid
    /// direction and custom-count choices live in the macOS View menu so this
    /// row stays compact without making the common cases slower.
    ...
    private var pluginToolbarMenu: some View {
        Menu {
                       ...
                       let action = plugin.manifest.actions.first {
                        Button {
                            ...
                            Label(
                                ...
                                systemImage: pluginActionSystemImage(action.output)
                            )
                        }
                        .disabled(!PluginCommandAvailability.isAvailable(
                            action,
                            workspace: workspace
                        ))
                        .help(
            ...
            .frame(height: 32)
            .background(
                workspace.pluginPanelRequest == nil ? theme.card : theme.dropHighlight,
        ...
        .accessibilityAddTraits(
            workspace.pluginPanelRequest == nil ? [] : .isSelected
        )
    }
    ...
    private func pluginActionSystemImage(_ output: PluginActionOutput) -> String {
        switch output {
    ...
    private var highlightSettingsPopover: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            ColorPicker(
                L10n.string("study.markup.color"),
    ...
    /// ColorPicker speaks SwiftUI `Color`, while PDF annotations use AppKit
    /// `NSColor`. Updating the existing tab-owned style keeps Viewer, Editing
    /// and Study in sync without introducing another preference to maintain.
    private var highlightColorBinding: Binding<Color> {
        Binding(
            get: { Color(nsColor: workspace.studyMarkupStyle.color) },
            set: { color in
                var style = workspace.studyMarkupStyle
                style.color = NSColor(color)
