...
final class AppMenuRegistrationTests: XCTestCase {
    func testApplicationCommandsAreRegisteredExactlyOnceForAllPDFWindows() throws {
        ...
        XCTAssertEqual(source.components(separatedBy: "HwattakPDFCommands(").count - 1, 1)
        ...
        XCTAssertEqual(
            source.components(separatedBy: "WindowGroup(\"HwattakPDF\", id: \"main\", for: MainWorkspaceWindowID.self)").count - 1,
            1,
            "The primary workspace must participate in external file routing."
        )
    }
    ...
    func testPluginsHaveAVisibleWorkspaceEntryPointAndSingleActionMenusAreFlat() throws {
        ...
        let commandSource = try String(
            contentsOf: Self.projectRoot
                .appendingPathComponent("Sources/HwattakPDF/App/PluginCommands.swift"),
            encoding: .utf8
        )
    ...
    func testNativeViewMenuOwnsDetailedWorkspacePresentationCommands() throws {
        ...
        XCTAssertTrue(source.contains("ForEach(PDFPageNavigationMode.allCases)"))
    ...
    func testUndoRedoCommandsRouteThroughTheFocusedWorkspaceSafely() throws {
        ...
        let routerSource = try String(
            contentsOf: Self.projectRoot
                .appendingPathComponent("Sources/HwattakPDF/App/AppEditCommandRouter.swift"),
            encoding: .utf8
        )
