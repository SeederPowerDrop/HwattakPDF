...
final class PDFWorkspaceModeTests: XCTestCase {
    ...
    func testDocumentWorkspaceDoesNotReserveABottomStatusBar() throws {
        let workspaceView = try source("Sources/HwattakPDF/Views/WorkspaceView.swift")
    ...
    func testTwoTierToolbarsKeepOverflowAndVoiceOverControlsDiscoverable() throws {
        ...
        let toolbar = try String(
            contentsOf: projectRoot
                .appendingPathComponent("Sources/HwattakPDF/Views/WorkspaceToolbar.swift"),
            encoding: .utf8
        )
        let studyPalette = try String(
            contentsOf: projectRoot
                .appendingPathComponent("Sources/HwattakPDF/Views/StudyModePalette.swift"),
            encoding: .utf8
        )
        let workspaceView = try String(
            contentsOf: projectRoot
                .appendingPathComponent("Sources/HwattakPDF/Views/WorkspaceView.swift"),
            encoding: .utf8
        )
    ...
    func testMutationSurfacesConsultCapabilitiesInsteadOfBypassingMode() throws {
        let sidebar = try source("Sources/HwattakPDF/Views/PageSidebarView.swift")
        let grid = try source("Sources/HwattakPDF/Views/PDFGridOverview.swift")
        let workspaceView = try source("Sources/HwattakPDF/Views/WorkspaceView.swift")
        let tabbedWorkspace = try source(
            "Sources/HwattakPDF/Views/TabbedWorkspaceView.swift"
        )
        let fileDrop = try source("Sources/HwattakPDF/Views/WorkspaceFileDrop.swift")
        let app = try source("Sources/HwattakPDF/App/HwattakPDFApp.swift")
        let annotationOverlay = try source(
            "Sources/HwattakPDF/Views/PDFImageEditingOverlay.swift"
        )
    ...
    func testFinderFileDropReceiverCoversTheFullWorkspaceBoundaryExactlyOnce() throws {
        ...
        let workspaceView = try source("Sources/HwattakPDF/Views/WorkspaceView.swift")
        let pageSidebar = try source("Sources/HwattakPDF/Views/PageSidebarView.swift")
        let grid = try source("Sources/HwattakPDF/Views/PDFGridOverview.swift")
